...
export default function AuthPage({ mode }: { mode: 'login' | 'register' }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authenticatedRole === 'Learner') navigate('/dashboard/learner/study')
      else if (authenticatedRole === 'Educator') navigate('/dashboard/educator/lectures')
      else if (authenticatedRole === 'Admin') navigate('/dashboard/admin')
  ...
  const handleGoogleAuth = async () => {
    ...
    setErrorMsg(null)
    try {
      const res = await api.loginWithGoogle('google_oauth_token_client_auth', role)
      setLoading(false)
      ...
      setLoading(false)
      setErrorMsg(err.message || 'Google OAuth service is unavailable. Please sign in or register with email.')
    }
  }
